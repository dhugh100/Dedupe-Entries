...
int main (int argc, char **argv)
{
	...
	udp->fdpp = g_malloc0((MAX_FOLDERS + 1) * sizeof(char *)); // Allocate folder memory, extra slot keeps it null terminated
	...
	// App setup
	// App id must match the installed desktop file name for Wayland to
	// associate windows with the launcher (icon, dock, alt-tab)
	GtkApplication *app = gtk_application_new("io.github.dhugh100.Dedupee", G_APPLICATION_DEFAULT_FLAGS);
	udp->app = app;

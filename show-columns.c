...
void show_columns (user_data *udp)
{
	...
	// - The new call takes ownership of a model reference, so pass our own
	// - Release any selection model from a prior show
	GtkMultiSelection *selection = gtk_multi_selection_new(g_object_ref(G_LIST_MODEL(udp->list_store)));
	if (udp->selection) g_object_unref(udp->selection);

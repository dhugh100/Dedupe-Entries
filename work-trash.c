...
void trash_em (user_data *udp)
{
	...
	// Loop through bitset, trashing each item - guarded so an empty
	// bitset trashes nothing
        for (gboolean valid = gtk_bitset_iter_init_first(&iter, udp->sel_bitset, &value); valid;

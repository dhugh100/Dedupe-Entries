...
{
	// And requires both matches, or requires either
	if (udp->fep->and)
		return subres(item, udp) && subname(item, udp);
	return subres(item, udp) || subname(item, udp);
}
...
void apply_filters_cb (GtkWidget *self, user_data *udp)
{
	...
	// Keep the filtered snapshot in step so the next filter compounds
...
{
	g_strlcpy(udp->fep->name_ebt, gtk_entry_buffer_get_text(udp->fep->name_eb), STR_ENTRY);
}
...
{
	g_strlcpy(udp->fep->res_ebt, gtk_entry_buffer_get_text(udp->fep->res_eb), STR_ENTRY);
}

...
int traverse (char *dir_str, user_data *udp)
{
	...
	while ((entry = readdir(dir))) {
		...
		if (entry->d_name[0] == '.' && udp->opt_include_hidden == FALSE) continue;
		...
		// Some filesystems do not fill in d_type, fall back to lstat
		unsigned char d_type = entry->d_type;
		if (d_type == DT_UNKNOWN && lstat(full_name, &attr) == 0) {
			if (S_ISDIR(attr.st_mode)) d_type = DT_DIR;
			else if (S_ISREG(attr.st_mode)) d_type = DT_REG;
		}
		...
		// Only working on POSIX regular files - symlinks are not followed
		if (d_type != DT_REG) continue;
		...
		if (g_list_model_get_n_items(G_LIST_MODEL(udp->list_store)) >= MAX_ENTRIES) {
			g_idle_add(cancel_clean_up_idle, udp); // treat as if cancel requested
			if (dir) closedir(dir);
		...
		// Create object to store - hashing is deferred to hash_candidates
		item = g_object_new(DUP_TYPE_ITEM, "name", full_name, "hash", "", NULL);

		res = stat(full_name, &attr);

		if (res == -1) { // -1 is error on stat
		...
		// Mark empty files, all others are hashed later by hash_candidates
		// if another file of the same size makes a duplicate possible
		if (attr.st_size == 0) g_object_set(item, "result", STR_EMP, NULL);
		g_list_store_append(udp->list_store, item);
		g_object_unref(item);
	} // End Dir read while   

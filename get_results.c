...
int get_results(GListStore *list_store, user_data *udp) 
{
	// Use quick sort - sort by hash to enable finding dup groups
	g_list_store_sort(list_store, (GCompareDataFunc) cmp_function, NULL);
	...
	if (cnt == 1) {
		item = g_list_model_get_item((GListModel *)list_store,0);
	...
	while ( (i+1) < cnt ) { // Since look ahead to determine group, last struct handled outside loop
		...
		item = g_list_model_get_item((GListModel *)list_store,i);
		...
		next_item =  g_list_model_get_item((GListModel *)list_store,i+1);

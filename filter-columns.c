// This file, filter_columns.c, is a part of the Dedupe Entries program.
// 
...
int filter_an_item(user_data *udp)
{
        g_list_store_remove(udp->list_store, udp->remove_position);
...
void apply_filters_cb (GtkWidget *self, user_data *udp)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		g_list_store_append (udp->saved_list_store, item);
	}

	gboolean see;
	g_list_store_remove_all (udp->list_store);
	// Update the list store based on filter
	for (i = 0; i < cnt; i++) {
		item = g_list_model_get_item (G_LIST_MODEL(udp->saved_list_store), i);
		see = filter_match (item, udp);
		if (see)
...
void clear_filters_cb (GtkWidget *self, user_data *udp)
{
	...
	udp->fep->or = FALSE;
 	...
 	guint32 i;
	guint32 cnt = g_list_model_get_n_items (G_LIST_MODEL(udp->saved_list_store));
        DupItem *item = g_object_new(DUP_TYPE_ITEM, NULL);
        for (i = 0; i < cnt; i++) {
                item = g_list_model_get_item (G_LIST_MODEL(udp->saved_list_store), i); 
                g_list_store_append (udp->list_store, item);

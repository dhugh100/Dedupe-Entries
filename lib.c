...
void cancel_clean_up (user_data *udp)
{
        ...
        // Clean up any data
        g_idle_add(clear_stores_idle, udp);
...
// Clear the stores
// - Items free their strings on finalize, so each store just drops its
//   references and shared items go away when the last store lets go
...
// Idle wrappers so g_idle_add gets sources with a proper gboolean return

gboolean clear_stores_idle (gpointer data)
{
        clear_stores(data);
        return G_SOURCE_REMOVE;
}

gboolean cancel_clean_up_idle (gpointer data)
{
        cancel_clean_up(data);
        return G_SOURCE_REMOVE;
}
...
{
        // If the file can't be stat'd there are no saved options
        struct stat attr;
        if (stat(name, &attr) == -1) {
		return FALSE;
	}
...
void option_init (user_data *udp)
{
        ...
        if (read_options(buff, udp->opt_name)) {
                ...
                // Don't trust a bad preserve value from the file, auto trash depends on it
                if (udp->opt_preserve >= AP_N) udp->opt_preserve = AP_SHORTEST;
	}
        else {

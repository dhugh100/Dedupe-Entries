...
int get_results (user_data *udp)
{
	...
	// Loop through sorted file to get unique groups assigned to each file based on hash
	char cghash[STR_HASH] = { 0x00 }; // Will store current group hash while looping
	...
	while ((i + 1) < cnt) { // Since look ahead to determine group, last struct handled outside loop
		...
		if (!strcmp((const char *)cghash, item->hash)) { // True if equal to current group hash
			snprintf(buff, sizeof(buff), "%07d", group);
			g_object_set(item, "result", buff, NULL);
		}
		else { // Not part of the current group, see if it starts a new one
			if (!strcmp(item->hash, next_item->hash)) { // True if next hash equal, means a group
				group++; // Found a group
				snprintf(buff, sizeof(buff), "%07d", group);
				g_object_set(item, "result", buff, NULL);

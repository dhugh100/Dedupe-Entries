...
// Comparison function for sort of result ascending modified time descending
...
int sort_modified_d (const void *a, const void *b)
{
	...
	DupItem *item2 = (DupItem *) b;
	if (!strcmp(item1->result, item2->result)) // Equal result on primary sort
		return strcmp(item2->modified, item1->modified); // Descending modified time sort (newest first)
	else
		return strcmp(item1->result, item2->result);
}
...
int sort_name_len_a (const void *a, const void *b)
{
	...
	DupItem *item2 = (DupItem *) b;
	if (!strcmp(item1->result, item2->result)) // Equal result on primary sort
		if (strlen(item1->name) < strlen(item2->name)) return -1; // Ascending name str len

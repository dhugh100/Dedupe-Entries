...
int count_selected_result (GtkBitset *bitset, GListStore *list_store, const char *result)
{
        ...
        // Guarded loop so an empty bitset iterates zero times
        for (gboolean valid = gtk_bitset_iter_init_first(&iter, bitset, &value); valid;
             valid = gtk_bitset_iter_next(&iter, &value)) {
...
void view_cb (GtkCheckButton *self, user_data *udp)
{
	...
	// Just view files - not a directory and not an error entry
...
void copy_cb (GtkCheckButton *self, user_data *udp)
{
	...
	while (gtk_bitset_iter_next(&iter, &value)) {
		...
		strncat(clip_text, work, STR_CLIP - (strlen(clip_text) + 1));
	}
...
void work_selected_file_cb (GtkGestureClick *self, int n_press, double x, double y, user_data *udp)
{
	...
	// Take the selection snapshot, releasing any prior one
	GtkBitset *sel_bitset = gtk_selection_model_get_selection((GtkSelectionModel *) udp->selection);
	if (udp->sel_bitset) gtk_bitset_unref(udp->sel_bitset);

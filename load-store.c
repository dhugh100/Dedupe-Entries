...
void cancel_cb (GtkWidget *self, user_data *udp)
{
	...
	g_idle_add(cancel_clean_up_idle, udp);
}
...
void load_entry_data (user_data *udp)
{
	...
	// - All folders must be loaded before results are determined so that
	//   duplicates spanning folders group together and group ids stay unique
	int loaded = 1;
	for (int i = 0; udp->fdpp[i]; i++) { // If fdpp[i] is NULL, then no more directories
		...
		if (!traverse(udp->fdpp[i], udp)) {
			loaded = 0; // 0 for show stopper
			break;
		}
	} // End for

	// With all folders loaded, hash the files that could have a duplicate
	// (same size as another file), then determine results in one pass
	if (loaded && hash_candidates(udp) && get_results(udp)) {

		// Check to see if result type should be included
